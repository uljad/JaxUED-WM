...
from jaxued.environments.gymnax.cartpole import CartPole, make_eval_levels_and_names as eval_levels_cp
from jaxued.environments.world_models.gymnax.cartpole import CartPoleWM, make_level_generator as make_level_generator_cp
...
def main(config=None, project="JAXUED_TEST"):
    ...
    if config['env'] == 'cartpole':
        env = CartPoleWM(config["model_paths"])
        eval_env = CartPole()
        sample_random_level = make_level_generator_cp(len(config["model_paths"]))
...
if __name__=="__main__":
    ...
    model_dir = "../WoMoCu/WorldModelsParams/cartpole_100/50"
    config["model_paths"] = [os.path.join(model_dir, model_name) for model_name in os.listdir(model_dir)][:10]
